...
def test_user_message_label_shown(page_with_message):
    """Input: a ticket with one submitted message. Expected: the 'YOU (Ticket Submitter)' label shows."""
...
def test_agent_message_label_shown(page_with_message):
    """Input: a ticket with an agent reply. Expected: the '[HELPDESK BOT] (Level 1 Support)' label shows."""
...
def test_user_message_text_shown(page_with_message):
    """Input: a ticket with one submitted message. Expected: the message text shows in the thread."""
    # Scope to the thread: the text also appears in the sidebar + header titles.
    expect(
        page_with_message.get_by_test_id("ticket-thread").get_by_text(
            "I need help processing my feelings about Mondays"
        )
    ).to_be_visible()


def test_agent_reply_text_shown(page_with_message):
    """Input: a ticket with an agent reply (mocked). Expected: the reply text shows."""
    expect(page_with_message.get_by_text(MOCK_REPLY)).to_be_visible()


def test_star_button_exists_in_dom(page_with_message):
    """Input: a ticket with an agent reply. Expected: a 'Star this response' button exists."""
...
def test_copy_button_exists_in_dom(page_with_message):
    """Input: a ticket with an agent reply. Expected: a 'Copy response' button exists."""
...
def test_star_message_changes_button_title(page_with_message):
    """Input: click Star on an agent reply. Expected: the button switches to 'Unstar'."""
...
def test_star_message_shows_starred_label(page_with_message):
    """Input: click Star on an agent reply. Expected: a 'Starred' label appears."""
    page_with_message.get_by_title("Star this response").click(force=True)
...
def test_unstar_message_restores_star_button(page_with_message):
    """Input: star then unstar an agent reply. Expected: the 'Star this response' button returns."""
    page_with_message.get_by_title("Star this response").click(force=True)
...
def test_copy_button_writes_to_clipboard(page_with_message):
    """Input: click Copy on an agent reply. Expected: the reply text is on the clipboard."""
...
def test_multiple_messages_all_displayed(page):
    """Input: submit two different messages. Expected: both appear in the thread."""
    messages = [
    ...
    for msg in messages:
        expect(thread.get_by_text(msg)).to_be_visible()


def test_multiple_agent_replies_displayed(page):
    """Input: submit two messages. Expected: two agent replies are shown."""
    for _ in range(2):
...
def test_user_message_has_no_star_or_copy_buttons(page_with_message):
    """Input: a ticket with a user message and an agent reply. Expected: star/copy buttons exist only on agent messages, not user ones."""
